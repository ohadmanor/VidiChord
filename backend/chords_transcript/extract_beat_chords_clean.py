...
def main():
    ...
    print("==========================================================")
    print("      CLEAN LIBROSA BEAT-LEVEL CHORD TRANSCRIPTION")
    print("==========================================================")
    ...
    # Run clean chord detection
    librosa_chords = get_clean_librosa_beat_chords(y, sr_load, beat_intervals)

    # Load ground truth if exists
    gt_filename = f"{os.path.splitext(os.path.basename(audio_path))[0]}_clean_beat_ground_truth.json"
    ...
    # Generate HTML Report using the existing beat report template
    ...
    out_html_filename = f"chord_beat_report_{os.path.splitext(os.path.basename(audio_path))[0]}_clean.html"
    out_html_path = os.path.join(os.path.dirname(audio_path), out_html_filename)
    with open(out_html_path, "w", encoding="utf-8") as f:
        f.write(rendered_html)
        
    print(f"SUCCESS: Clean beat-level report generated successfully!")
